...
//multiply two polynomials over Zq a being in standard canonical representation, b in ntt representation with reduction polynomial X^n+1
function ZKNOX_NTT_MUL(
       uint[] memory a,
       uint[] memory b
    ) public view returns (uint[] memory){
         return (ZKNOX_NTTINV(ZKNOX_VECMULMOD(ZKNOX_NTTFW(a, o_psirev), ZKNOX_NTTFW(b, o_psirev), storage_q), o_psi_inv_rev));
    }
