...
function F1600(uint64[25] memory state) public view returns (uint64[25] memory){
 ...
 for (uint i=0;i<24;i++){ //range(24):
      uint64 t;
      //# Parity
      for (uint x=0;x<5;x++){  //range(5):
        bc[x] = 0;
        for(uint y=0;y<25;y+=5)// range(0, 25, 5):
          bc[x] ^= state[x + y];
     }
     //# Theta
      for(uint x=0;x<5;x++){  //range(5):
        t = bc[(x + 4) % 5] ^ rol64(bc[(x + 1) % 5], 1);
        for(uint64 y=0;y<25;y+=5)// in range(0, 25, 5):
          state[y + x] ^= t;
      }
      ...
      for (uint y=0;y<25;y+=5){// in range(0, 25, 5):
       for (uint x=0;x<5;x++)  //range(5):
          bc[x] = state[y + x];
        for (uint x=0;x<5;x++){  //range(5):
